...
(function() {
  ...
  var queryDialog =
    'Искать: <input type="text" style="width: 10em"/> <span style="color: #888">(используйте /re/ синтаксис для поиска регулярных выражений)</span>';
  function doSearch(cm, rev) {
    var state = getSearchState(cm);
    if (state.query) return findNext(cm, rev);
    dialog(cm, queryDialog, "Поиск:", function(query) {
  ...
  function replace(cm, all) {
    dialog(cm, replaceQueryDialog, "Найти для замены:", function(query) {
      ...
      dialog(cm, replacementQueryDialog, "Заменить на:", function(text) {
        if (all) {
