...
RooCMS 2.0 Alpha представляет собой крупное обновление с полным переписыванием архитектуры. Основной фокус на современные технологии, безопасность, производительность и API-first подход для создания не только веб-сайтов, но и любых приложени
...
- ➕ **Темы и шаблоны** - система тем для фронтенда
...
#### API и Архитектура
- ➕ **RESTful API система** - Полнофункциональный REST API с версионированием (`/api/v1/`)
- ➕ **ApiHandler класс** - Собственная система роутинга с поддержкой динамических параметров
- ➕ **Middleware система** - Гибкая система промежуточного ПО для аутентификации и авторизации
- ➕ **Factory Pattern** - Контроллеры и middleware создаются через фабрики для лучшей архитектуры

#### База данных
- ➕ **Query Builder** - Мощный конструктор запросов с защитой от SQL инъекций
- ➕ **Универсальная система миграций** - CLI инструмент с поддержкой MySQL, PostgreSQL, Firebird
- ➕ **Database Health Monitoring** - Мониторинг состояния БД с детальной диагностикой
- ➕ **Мульти-база поддержка** - Работа с несколькими базами данных одновременно
- ➕ **Система констант таблиц** - Элегантное управление именами таблиц через константы

#### Безопасность и аутентификация
- ➕ **Современная система Auth** - Токены, роли, верификация кодов
- ➕ **CSP поддержка** - Content Security Policy для защиты от XSS
- ➕ **Строгая валидация** - Комплексная система проверки и санитизации данных

### 🔄 Обновления и улучшения

#### Архитектурные изменения
- 🔄 **PHP 8.1+ совместимость** - Полный переход на современные возможности PHP
- 🔄 **Строгая типизация** - Все методы используют type hints и return types
- 🔄 **Отказ от фреймворков** - Чистый PHP код без внешних зависимостей
- 🔄 **Модульная архитектура** - Четкое разделение на классы, сервисы и контроллеры

#### База данных
- 🔄 **Расширенная SQL поддержка** - MySQL, PostgreSQL, Firebird в едином коде
- 🔄 **Оптимизированные запросы** - Подсчет и мониторинг производительности
- 🔄 **Транзакционная безопасность** - Все операции выполняются в транзакциях

### 🐛 Исправления

- 🐛 **Безопасность запросов** - Устранены потенциальные SQL инъекции через Query Builder
- 🐛 **Обработка ошибок** - Централизованная система обработки ошибок в API
- 🐛 **Совместимость типов данных** - Корректная работа с разными типами БД

### ⚠️ Важная информация

- ℹ️ **Обратная совместимость** - Старые vanilla запросы временно поддерживаются, но будут удалены в версии 2.1
...
- ❌ **Поддержка PHP < 8.1** - Минимальная версия PHP теперь 8.1
- ❌ **Старые методы БД** - Устаревшие методы работы с базой данных (будут удалены в 2.1)
- ❌ **Небезопасные практики** - Удалены все методы, допускающие SQL инъекции
...
- 🗄️ [Система миграций](roocms/database/README.md)
