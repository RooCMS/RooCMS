...
class REPASS {
	...
	private function reminder() {
		...
		if(isset($POST->email) && $parse->valid_email($POST->email) && $db->check_id($POST->email, USERS_TABLE, "email")) {
			...
			# уведомление
			$logger->info("Инструкции для восстановления пароля, отправлены Вам на электронную почту");
			...
			# bad result
			$logger->error("Невозможно выполнить запрос на восстановление пароля. Мы не нашли данных о Вашей учетной записи.");
			goback();
		}
	}
