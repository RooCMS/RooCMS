...
<div
    ...
    class="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-gray-200/50 shadow-2xl"
    style="display: none;"
    x-init="init()"
>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('cookieConsent', () => ({
        ...
        checkConsentStatus() {
            ...
            const isRejected = localStorage.getItem('roocms_cookie_rejected');
        ...
        acceptAll() {
            try {
                ...
                localStorage.setItem('roocms_cookie_preferences', JSON.stringify(preferences));
        ...
        reject() {
            try {
                ...
                localStorage.removeItem('roocms_cookie_consent_given');

                this.isRejected = true;
            } catch (error) {
                console.warn('Failed to save cookie rejection:', error);
            }
        },

        resetRejection() {
            try {
                // Clear rejection and allow user to choose again
                localStorage.removeItem('roocms_cookie_rejected');
                this.isRejected = false;
            } catch (error) {
                console.warn('Failed to reset cookie rejection:', error);
            }
        }
    }));
});
