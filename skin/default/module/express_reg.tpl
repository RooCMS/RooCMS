			...
			{else}
				<form method="post" action="{$SCRIPT_NAME}?part=reg&act=expressreg">
					...
					{if isset($config->fl152_use) && $config->fl152_use}
						<p class="small text-center mb-2">Совершая подписку Вы соглашаетесь <nobr>с <a href="{$SCRIPT_NAME}?part=fl152&ajax=true" data-fancybox data-animation-duration="300" data-type="ajax"><b>условиями передачи информации</b></a></nobr></p>
					{/if}
