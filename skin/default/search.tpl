					...
					<div class="row no-gutters">
						{if isset($item['image'][0])}
							<div class="col-md-5 col-xl-4">
								{foreach from=$item['image'] item=image}
									<a href="{$SCRIPT_NAME}?page={$item['alias']}{if isset($smarty.get.pg)}&pg={$smarty.get.pg}{/if}&id={$item['id']}{if isset($smarty.get.search)}&search={$smarty.get.search}{/if}" title="{$item['title']}"><img src="upload/images/{$image['thumb']}" class="card-img" alt="{$image['alt']}"></a>
								{/foreach}
							</div>
						{/if}
						<div class="col-md-{if isset($item['image'][0])}7{else}12{/if} col-xl-{if isset($item['image'][0])}8{else}12{/if}{if isset($item['image'][0])} roocms-feedbrief-mh{/if}">
							<div class="card-body d-flex flex-column mh-100 h-100">
								<h5 class="card-title mb-1"><a href="{$SCRIPT_NAME}?page={$item['alias']}&id={$item['id']}{if isset($smarty.get.search)}&search={$smarty.get.search}{/if}">{$item['title']}</a></h5>
								...
								<div class="card-text small mt-auto">
									...
									<a href="{$SCRIPT_NAME}?page={$item['alias']}&id={$item['id']}{if isset($smarty.get.search)}&search={$smarty.get.search}{/if}" class="text-secondary text-uppercase float-right">Читать<span class="fas fa-chevron-circle-right fa-fw"></span></a>
								</div>
							</div>
						</div>
					</div>
