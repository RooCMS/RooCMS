...
<div class="row">
	...
	<div class="{if $userdata['avatar'] != ""}col-xs-9 col-sm-10 col-md-10{else}col-xs-12{/if}">
		...
		<div class="ucp-user-slogan text-truncate">{$userdata['user_slogan']}</div>
		...
		<form method="post" action="{$SCRIPT_NAME}?part=ucp&act=ucp&move=update_info" role="form" enctype="multipart/form-data">
			...
			<fieldset>
				...
				<div class="form-group">
					...
					<div class="col-lg-8">
						{if $userdata['avatar'] != ""}<div class="btn-group pull-right" data-toggle="buttons"><label class="btn btn-default btn-sm"><input type="checkbox" name="delete_avatar" class="ida" value="1" autocomplete="on"><i class="iconda fa fa-fw fa-trash"></i> Удалить аватар?</label></div>{/if} <input type="file" name="avatar" id="inputAvatar" class="btn btn-default">
					</div>
				</div>
