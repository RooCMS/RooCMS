...
<div class="row">
    ...
    <div class="row">
		{if isset($item['image'][0])}
			...
			<div class="col-sm-2 text-center">
				<a href="{$SCRIPT_NAME}?page={$feed['alias']}{if isset($smarty.get.pg)}&pg={$smarty.get.pg}{/if}&id={$item['id']}"><img src="upload/images/thumb/{$image['filename']}" border="0" alt="{$image['alt']}" class="img-thumbnail"></a>
			</div>
			{/foreach}
		{/if}
    ...
    <div class="row">
    	<div class="col-sm-12 text-right">
			<a href="{$SCRIPT_NAME}?page={$feed['alias']}{if isset($smarty.get.pg)}&pg={$smarty.get.pg}{/if}&id={$item['id']}" class="btn btn-xs btn-primary">Читать полностью <span class="icon-arrow-right icon-fixed-width"></span></a>
		</div>
	</div>
	</div>
	{/foreach}
