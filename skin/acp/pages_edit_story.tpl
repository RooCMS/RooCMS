...
<form method="post" action="{$SCRIPT_NAME}?act=pages&part=update&page={$data['sid']}" enctype="multipart/form-data" role="form">
	<div class="card-body">
		...
		{foreach from=$datacontent item=content name=content}
			<input type="hidden" name="sort[{$content['id']}]" value="{$content['sort']}">
			<div class="form-group">
				<span class="badge badge-light">Блок # {$content['sort']}</span>
				<textarea id="content_field" class="form-control ckeditor" name="content[{$content['id']}]" spellcheck>{$content['content']}</textarea>
			</div>

			<div class="row">
				<div class="col-md-6">
					{$attachedimages[$content['id']]}
				</div>
				<div class="col-md-6">
					{$attachedfiles[$content['id']]}
				</div>
			</div>
