	...
	{if !empty($subtree)}
		<div class="card-body">
			{foreach from=$subtree item=subpart key=k name=helptrees}
				...
				{/if}
				<a href="{$SCRIPT_NAME}?act=help&u={$subpart['uname']}" class="list-group-item list-group-item-action{if $subpart['level'] == 0} text-bold{/if}">{section name=level loop=$subpart['level']}&bull;{/section}&bull; {$subpart['title']}</a>
