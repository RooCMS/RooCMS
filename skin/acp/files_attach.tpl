...
{foreach from=$attachfile item=file}
	<p class="bg-success attach_files"  id="filea-{$file['id']}">
		<a href="/upload/files/{$file['file']}" id="afile-{$file['id']}" target="_blank">
			{if file_exists("skin/acp/img/icon/16/{$file['fileext']}.png")}<img src="skin/acp/img/icon/32/{$file['fileext']}.png" border="0" width="32" height="32">{else}<span class="fa fa-file-code-o fa-fw fa-2x" rel="tooltip" title="{$type['mime_type']}" data-placement="top"></span>{/if}{$file['file']}
		</a>
