	...
	<ul class="nav nav-pills nav-stacked">
	    ...
	    {foreach from=$tree item=gpart key=k}
		    {if $gpart['level'] == 1 && $k != 0}
				<li{if isset($smarty.get.u) && $smarty.get.u == $gpart['uname']} class="active"{/if}><a href="{$SCRIPT_NAME}?act=help&u={$gpart['uname']}"><span class="fa fa-fw fa-stethoscope"></span> {$gpart['title']}</a></li>
			{*
			{elseif $k != 0}
